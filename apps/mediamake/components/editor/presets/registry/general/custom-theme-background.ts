/**
 * Custom Theme Background Preset
 *
 * This preset creates customizable themed backgrounds with color palettes, patterns, gradients,
 * and media overlays. It provides extensive styling options for creating visually appealing backgrounds.
 *
 * Features:
 * - **Color Palettes**: Predefined palettes (sunset, ocean, neon, forest, etc.) or custom colors
 * - **Background Types**: Solid, radial gradient, image overlay, or video overlay
 * - **Pattern Overlays**: Dots, stripes, grid, or checkerboard patterns
 * - **Media Overlays**: Image or video backgrounds with opacity and blend mode controls
 * - **Animation Support**: Optional animated gradients and transitions
 * - **Flexible Styling**: Extensive customization options for all background elements
 *
 * Use cases:
 * - Creating themed backgrounds for video compositions
 * - Building branded backgrounds with custom colors
 * - Adding visual interest with patterns and gradients
 * - Creating dynamic backgrounds with media overlays
 */
...
const presetParams = z.object({
  ...
  backgroundType: z
    .enum(['solid', 'radial-gradient', 'image-overlay', 'video-overlay'])
  ...
  patternGlow: z.boolean().optional().describe('Enable glow effect on pattern'),
  ...
  mediaUrl: z
    .string()
    .optional()
    .describe('Image or video URL for media overlays'),
  ...
  fixedDuration: z.number().optional().describe('Fixed duration in seconds'),
...
const presetExecution = (
  ...
  const generatePattern = (
    colors: string[],
  ): { pattern: string; size: string; opacity: number; filter?: string } => {
  ...
  const generateBackgroundStyle = (): React.CSSProperties => {
    switch (backgroundType) {
      ...
      case 'radial-gradient':
        return {
          backgroundImage: generateRadialGradient(activeColors),
          zIndex: 0,
        };
      default:
        return { backgroundColor: activeColors[0], zIndex: 0 };
  ...
  if (backgroundType === 'image-overlay' && mediaUrl) {
    ...
    enablePattern &&
    (backgroundType === 'solid' || backgroundType === 'radial-gradient')
  ) {
