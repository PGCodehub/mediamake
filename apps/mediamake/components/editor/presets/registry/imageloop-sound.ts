...
const presetExecution = (
  ...
  const imageComponents = images.map((image, imageIndex) => {
    ...
    if (hasStackEffect) {
      ...
      const maintainAspectRatio = stackEffect?.stack?.maintainAspectRatio ?? false;
      ...
      if (arrangement === 'dynamic') {
        ...
        if (imageIndex === 0) {
          ...
          initialLayout = calculateDynamicLayout(numImagesAtArrival, imageIndex, gap);
        }
        ...
        for (let futureImageIndex = imageIndex + 1; futureImageIndex < totalImages; futureImageIndex++) {
          const numVisibleImages = futureImageIndex + 1;
          const prevNumVisibleImages = futureImageIndex;
          
          // Calculate current and new layouts
          const currentLayout = calculateDynamicLayout(prevNumVisibleImages, imageIndex, gap);
          const newLayout = calculateDynamicLayout(numVisibleImages, imageIndex, gap);
          
          const transitionTime = transitionTimings[futureImageIndex]?.start || 0;
          const relativeStart = transitionTime - (transitionTimings[imageIndex]?.start || 0);
          ...
          dynamicEffects.push({
            ...
            data: {
              mode: 'provider',
              targetIds: [`${params.trackName ?? 'imageloop-sound'}-image-${imageIndex}`],
              type: 'ease-out',
              ranges: [
      ...
      switch (direction) {
        case 'bottom':
          slideInEffect.ranges = [
            { key: 'transform', val: `translateY(${transformDistance})`, prog: 0 },
        ...
        case 'left':
          slideInEffect.ranges = [
            { key: 'transform', val: `translateX(-${transformDistance})`, prog: 0 },
      ...
      const effectiveImageDuration = transitionTimings[imageIndex]?.duration || 0.5;
      const startTime = transitionTimings[imageIndex]?.start || 0;
      
      // For stack effect, calculate total video duration
      const totalDuration = hasStackEffect && images.length > 0
        ...
        data: {
          ...
          style: {
            ...
            ...(useDynamicResize && imageIndex > 0 ? {
              transform: direction === 'bottom' ? 'translateY(150%)' :
                         direction === 'top' ? 'translateY(-150%)' :
                         direction === 'left' ? 'translateX(-150%)' :
                         'translateX(150%)',
              opacity: 0,
            } : {}),
          },
