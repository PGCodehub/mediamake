    ...
    return words.map((word, _j: number) => {
      ...
      if (animationStyle === 'gradient-wave-float') {
        effects.push({
          ...
          componentId: 'generic',
          data: createWaveFloatEffect(wordId, word, impact * (globalImpact || 1.0)),
        });
        ...
        effects.push({
          ...
          componentId: 'generic',
          data: createShimmerSlideEffect(wordId, word, _j, impact * (globalImpact || 1.0)),
        });
        ...
        effects.push({
          id: `pulse-${wordId}`,
          componentId: 'generic',
          data: createPulseEffect(wordId, word, impact * 0.5 * (globalImpact || 1.0)),
        });
        ...
        effects.push({
          ...
          componentId: 'generic',
          data: createMetallicShineEffect(wordId, word, impact * (globalImpact || 1.0)),
        });
      } else if (animationStyle === 'gradient-aurora-drift') {
        effects.push({
          id: `aurora-drift-${wordId}`,
          componentId: 'generic',
          data: createAuroraDriftEffect(wordId, word, impact * (globalImpact || 1.0)),
        });
        ...
        effects.push({
          id: `scale-${wordId}`,
          componentId: 'generic',
          data: createScaleEffect(wordId, word, impact * 0.8 * (globalImpact || 1.0)),
        });
      }
      ...
      return {
        ...
        data: {
          ...
          font: {
            family: fontFamily,
            weights: fontStyle.fontWeight ? [String(fontStyle.fontWeight)] : ['400', '700'],
          },
        } as TextAtomData,
        context: {
          timing: {
